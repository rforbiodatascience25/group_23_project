...
```{r}
raw_data_sv <- read_tsv(here("data", "sv_cleaned"))
...
glioma_SV |> 
  ggplot(aes(x = Site2_Hugo_Symbol, fill = Site1_Hugo_Symbol)) +
  geom_bar(color = "black", alpha = 0.8) +
  coord_flip() +
  theme_minimal() 

glioma_SV |> 
  mutate(Group = ifelse(Site1_Hugo_Symbol == "EGFR", "EGFR", "Other_genes"), Gene_Name = Site1_Hugo_Symbol) |> 
  ggplot(aes(x = Group, fill = Site1_Hugo_Symbol)) +
  geom_bar(size = 0.2) +
  scale_fill_viridis(option = "viridis",
                     discrete = TRUE) +
  theme_minimal() 
...
glioma_SV_longer |> 
  mutate(Group = ifelse(Gene_Name == "EGFR", "EGFR", "Other_genes"), Gene_Name = Gene_Name) |> 
